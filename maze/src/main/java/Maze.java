...
//        this.setLocationRelativeTo(null);
        ...
        new Thread(() -> {
            ...
            switch (this.algorithm) {
                ...
                case Definitions.ALGORITHM_DFS:
                    result = dfsAlgo(this.startRow, this.startColumn);
    ...
    public boolean isGoalFound(int x, int y) {
        return x == values.length - 1 && y == values.length - 1;
    }

    public boolean dfsAlgo(int x, int y) {
        ...
        if (!visited[x][y]) {
            ...
            if (y < values.length - 1) {
                if (!(visited[x][y + 1]) && values[x][y + 1] == Definitions.EMPTY) {
                    if (dfsAlgo(x, y + 1)) {
                        return true;
                    }
                }
            ...
            if (x < values.length - 1) {
                if (!(visited[x + 1][y]) && values[x + 1][y] == Definitions.EMPTY) {
                    if (dfsAlgo(x + 1, y)) {
                        return true;
                    }
                }
